...
export class CategoriaService {
  ...
  // Crear un registro
  async create(data: CreateCategoriaDto): Promise<GenericResponse<any>> {
    try {
      const createdData = await this.prisma.write.categoria.create({
        data: {
          ...data,
          registerActive: true,
        },
  ...
  async findAll(filterDto: PaginationDto): Promise<GenericResponse<any>> {
    try {
      ...
      const [data, total] = await Promise.all([
        this.prisma.read.categoria.findMany({

          skip: offset,
          take: limit,
        }),
        this.prisma.read.categoria.count({}),
      ]);
  ...
  async findOne(id: number): Promise<GenericResponse<any>> {
    try {
      const data = await this.prisma.read.categoria.findUnique({
        where: { id },
      });
      if (!data) {
        return new GenericResponse({
  ...
  async update(id: number, data: CreateCategoriaDto): Promise<GenericResponse<any>> {
    try {
      const updatedData = await this.prisma.write.categoria.update({
        where: { id },
        data: {
          ...data,
          updatedAt: new Date(),
  ...
  async changeStatus(id: number, isDeleted: boolean): Promise<GenericResponse<any>> {
    try {
      const deletedData = await this.prisma.write.categoria.update({
        where: { id },
        data: {
