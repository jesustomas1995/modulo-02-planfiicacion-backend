...
@ApiTags('CATALOGO - MARCA')
@Controller('/catalogo/marca')
@ApiSecurity('Authorization')
export class MarcaController {
  constructor(private readonly service: MarcaService) { }

  @Post()
  @ApiOperation({ summary: 'Servicio para registrar `Marca`' })
  ...
  @Post('list')
  @ApiOperation({ summary: 'Servicio para listar los `Marca` por pagination y filtros' })
  @ApiResponse({ status: 200, description: 'Operación exitosa' })
  @ApiResponse({ status: 400, description: 'Error de validación de datos' })
  @ApiResponse({ status: 500, description: 'Error Interno del servidor' })
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Servicio para obtener el `Marca` del aviso por ID' })
  @ApiResponse({ status: 200, description: 'Operación exitosa' })
  @ApiResponse({ status: 400, description: 'Error de validación de datos' })
  @ApiResponse({ status: 500, description: 'Error Interno del servidor' })
  ...
  @Put(':id')
  @ApiOperation({ summary: 'Servicio para actualizar la información del `Marca`' })
  @ApiResponse({ status: 200, description: 'Operación exitosa' })
  @ApiResponse({ status: 400, description: 'Error de validación de datos' })
  @ApiResponse({ status: 500, description: 'Error Interno del servidor' })
  update(
    @Param('id', ParseIntPipe) id: number,
    @Body() data: CreateMarcaDto,
  ) {
    return this.service.update(id, data);
  }

  @Delete(':id')
  @ApiOperation({ summary: 'Servicio para eliminar el registro de `Marca`' })
  @ApiResponse({ status: 200, description: 'Operación exitosa' })
  @ApiResponse({ status: 400, description: 'Error de validación de datos' })
  @ApiResponse({ status: 500, description: 'Error Interno del servidor' })
