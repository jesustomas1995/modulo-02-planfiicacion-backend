...
export class PresupuestoService {
  private model = "presupuesto"
  constructor(
    readonly prisma: PrismaService,
  ) { }

  // Crear un registro
  async create(data: CreatePresupuestoDto): Promise<GenericResponse<any>> {
    try {
  ...
  async findAll(filterDto: PaginationDto): Promise<GenericResponse<any>> {
    try {
      ...
      const [data, total] = await Promise.all([
        this.prisma.read.presupuesto.findMany({

          skip: offset,
          take: limit,
        }),
        this.prisma.read.presupuesto.count({}),
      ]);
  ...
  async findOne(id: number): Promise<GenericResponse<any>> {
    try {
      const data = await this.prisma.read.presupuesto.findUnique({
        where: { id },
      });
      if (!data) {
        return new GenericResponse({
  ...
  async update(id: number, data: CreatePresupuestoDto): Promise<GenericResponse<any>> {
    try {
      const updatedData = await this.prisma.write.presupuesto.update({
        where: { id },
        data: {
          ...data,
          updatedAt: new Date(),
  ...
  async changeStatus(id: number, isDeleted: boolean): Promise<GenericResponse<any>> {
    try {
      const deletedData = await this.prisma.write.presupuesto.update({
        where: { id },
        data: {
